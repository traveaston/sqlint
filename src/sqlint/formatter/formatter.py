...
class JoinFormatter(Formatter):
    ...
    def _format(cls, tree: SyntaxTree, stlye: str):
        for leaf in tree.leaves:
            join_indexes = [
                i for i, tk in enumerate(leaf.tokens) if tk == cls.join_token
            ]
            ...
            for idx in join_indexes:
                ...
                if adjusted_idx == 0:
                    leaf.tokens.insert(
                        adjusted_idx,
                        Token(
                            word=format_keyword("INNER", stlye),
                            kind=Token.KEYWORD,
                        ),
                    )
                    insert_count += 1
                ...
                if prev_token in [
                    ...
                    # When prev_token is 'OUTER' and previous it is not LEFT,
                    # RIGHT or FULL, I can't determined to insert which of
                    # these, so I ignore this illegal case.
                    continue
                if prev_token in [
                    cls.left_token,
                    cls.right_token,
                    ...
                    # When 'LEFT JOIN', 'RIGHT JOIN' or 'FULL JOIN', format as
                    # 'XXX OUTER JOIN'
                    leaf.tokens.insert(
                        adjusted_idx,
                        Token(
                            word=format_keyword("OUTER", stlye),
                            kind=Token.KEYWORD,
                        ),
                    )
                    insert_count += 1
                ...
                leaf.tokens.insert(
                    adjusted_idx,
                    Token(
                        word=format_keyword("INNER", stlye), kind=Token.KEYWORD
                    ),
                )
                insert_count += 1
...
class CommaPositionFormatter(Formatter):
    ...
    def _format_end(cls, tree: SyntaxTree):
        for idx, leaf in enumerate(tree.leaves):
            ...
            if idx > 0:
                tree.leaves[idx - 1].tokens.insert(
                    len(tree.leaves), poped_token
                )
        ...
        for leaf in tree.leaves:
            ...
            if tokens[0].kind == Token.WHITESPACE:
                ...
                leaf.node.insert(
                    0,
                    Token(
                        word=indent * (leaf.depth - 1), kind=Token.WHITESPACE
                    ),
                )
...
class WhiteSpacesFormatter(Formatter):
    ...
    def format_tokens(tokens: List[Token]):
        ...
        for idx, token in enumerate(tokens):
            ...
            if (
                next_tokens.kind
                in [Token.COMMA, Token.DOT, Token.BRACKET_RIGHT]
            ) or (
                token.kind == Token.IDENTIFIER
                and next_tokens.kind == Token.BRACKET_LEFT
            ):
                continue
