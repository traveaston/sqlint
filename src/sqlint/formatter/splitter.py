...
class Splitter(Generic[T], metaclass=ABCMeta):
    ...
    ) -> Tuple[List[T], List[List[T]], List[T]]:
    ...
    def split_other(
        ...
        for idx, token in enumerate(tokens):
            ...
            if bracket_count == 0 and token.kind in [
                Token.COMMA,
                Token.COMMENT,
            ]:
...
class IdentifierSplitter(Splitter):
    @classmethod
    def split(
        cls, tokens: List[Token], tree: SyntaxTree
    ) -> Tuple[List[Token], List[List[Token]], List[Token]]:
        ...
        for idx, token in enumerate(tokens[left_index + 1 :]):
            ...
            if bracket_count == 0:
                return (
                    ...
                    KeywordSelectSplitter.split_leaves(
    ...
    def split(
        ...
        if parent_tree.depth == 0:
            ...
            if tokens[0].kind != Token.COMMA:
                raise ValueError(
                    f'a head of tokens must be "," sequence, but {tokens[0]}'
                )

            if tokens[1].kind != Token.IDENTIFIER:
                # TODO: raises SQL error or check this as Violations
                raise ValueError(
                    f'next of "WITH" or "," must be identifier, but {tokens[1]}'
                )
            ...
            if tokens[3].kind != Token.BRACKET_LEFT:
                # TODO: raises SQL error or check this as Violations
                raise ValueError(
                    f'next of "AS" must be "(" in "WITH" or "," sequence, but {tokens[3]}'
                )
...
class KeywordSplitter(Splitter):
    ...
    def split(
        ...
        if head.kind not in [Token.KEYWORD, Token.FUNCTION]:
            raise ValueError(
                f"token kind must be reserved KEYWORD or FUNCTION, but {head.kind}"
            )
        ...
        key_functions = [
            ...
            (
                ...
                KeywordJoinSplitter.split_join,
            ),
        ]
    ...
    def _split_with(
        ...
        if tokens[1].kind != Token.IDENTIFIER:
            # TODO: raises SQL error or check this as Violations
            raise ValueError(
                f'next of "WITH" must be identifier, but {tokens[1]}'
            )
        ...
        if tokens[3].kind != Token.BRACKET_LEFT:
            # TODO: raises SQL error or check this as Violations
            raise ValueError(
                f'next of "AS" must be "(" in "WITH" sequence, but {tokens[3]}'
            )
    ...
    def _split_case(
        ...
        try:
            ...
            # TODO: raises SQL error or check this as Violations
            raise ValueError(
                '"CASE" sequence requires one or more "WHEN" sequense'
            )
        ...
        for idx, token in enumerate(tokens[when_index:]):
            ...
            if token == end_token and case_count == 1 and bracket_count == 0:
                return (
                    tokens[0:when_index],
                    [tokens[when_index : idx + when_index]],
                    tokens[idx + when_index :],
                )
...
class KeywordHavingSplitter(KeywordSplitter):
    ...
    def split_having(
        ...
        return (
            tokens[0:1],
            KeywordWhereSplitter.split_condiction(tokens[1:]),
            [],
        )
